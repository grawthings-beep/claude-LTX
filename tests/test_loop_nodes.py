...
class LoopNodeTests(unittest.TestCase):
    ...
    def test_phase_cut_produces_exact_six_second_cycle(self):
        self.assertEqual(self.nodes.phase_cut_output_frames(152, 8), 144)

    def test_phase_cut_rejects_invalid_blends(self):
        with self.assertRaises(ValueError):
            self.nodes.phase_cut_output_frames(15, 2)
        with self.assertRaises(ValueError):
            self.nodes.phase_cut_output_frames(152, 1)
        with self.assertRaises(ValueError):
            self.nodes.phase_cut_output_frames(16, 8)
