...
def resolve_download_url(url, headers, timeout=90):
    request_headers = headers | {
        "User-Agent": headers.get("User-Agent", "claude-ltx-template")
    }
...
def run_urllib(url, output, headers):
    ...
    request = urllib.request.Request(url, headers=headers | {"User-Agent": headers.get("User-Agent", "claude-ltx-template")})
