...
# Keep anal regions visible by default. Users can opt in by adding ``anus``.
...
def _load_model(model_name: str):
    ...
    if not model_path.is_file():
        raise FileNotFoundError(
            ...
            "Redeploy with DOWNLOAD_MODELS=1 and CIVITAI_API_TOKEN configured."
        )
...
def _selected_class_ids(names, requested: str) -> list[int]:
    ...
    selected = {
        item.strip().lower() for item in str(requested).split(",") if item.strip()
    }
...
class WanAutoMosaicVideo:
    """Segment explicit contours and mosaic an IMAGE batch after generation."""

    CATEGORY = "LTX/Post Processing"
    FUNCTION = "apply"
    RETURN_TYPES = ("IMAGE",)
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "WanAutoMosaicVideo": "LTX Auto Mosaic JUST Segmentation (CPU)"
}
